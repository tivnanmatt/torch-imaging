...
class BilinearInterpolator(SparseLinearOperator):
    def __init__(self, num_row, num_col, interp_points, device='cpu'):
        ...
        It inherits from the SparseLinearOperator class.
        ...
        self.wa = (self.ix1.type(torch.float32) - ix) * (self.iy1.type(torch.float32) - iy).view(1, 1, -1)
        self.wb = (ix - self.ix0.type(torch.float32)) * (self.iy1.type(torch.float32) - iy).view(1, 1, -1)
        self.wc = (self.ix1.type(torch.float32) - ix) * (iy - self.iy0.type(torch.float32)).view(1, 1, -1)
        self.wd = (ix - self.ix0.type(torch.float32)) * (iy - self.iy0.type(torch.float32)).view(1, 1, -1)
